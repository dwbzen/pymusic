...
def main():
    description = dedent("""
    Create and run a Markov Chain, producing words.
    """)
    ...
    parser.add_argument("--text", "-t", help="in-line text input. One of --text or --source must be specified")
    parser.add_argument("--source", "-s", help="input file name")
    parser.add_argument("--ignoreCase", "-i", help="ignore input case", action="store_true", default=False)
    ...
    parser.add_argument("--num", "-n", help="Number of words to produce",  type=int, default=10)
    ...
    parser.add_argument("--list", "-l", help="Display produce Words in order produced, default is false", action="store_true",default=False)
    parser.add_argument( "--postprocessing", "-p", help="post-processing: TC = title case, UC = upper case, LC = lower case. Default is None", default=None)
